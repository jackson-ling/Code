...
// 注意定义时为字符类型，为了实现输入的是整数，这里代码稍作修改，思路是不变的
void createbst(bstree &t)
{
    t = NULL;
    char ch;
    ...
    {
        if (ch == '\n' || ch == ' ')
            continue;           // 需要考虑：忽略换行和空格
        insertbst(t, ch - '0'); // 这里把字符数字转成整数
    }
}
// 删除算法（采用寻找前驱的方法）
...
void fillCanvas(bstree t, int r, int c)
{
    if (!t)
        return;
    // 假设 t->data 是 int 类型
    char buf[4]; // 用来存数字字符串，最多三位+结束符
    snprintf(buf, sizeof(buf), "%d", t->data);
    ...
    if (t->rchild)
    {
        if (r + 1 < MAXH && c + (int)strlen(buf) < MAXW)
            canvas[r + 1][c + (int)strlen(buf)] = '\\';
        fillCanvas(t->rchild, r + 2, c + (int)strlen(buf));
    }
}
...
int main()
{
    ...
    inodertraverse(t);
    printf("\n");
    // 说明：在创建算法中做了处理，定义的是char类型，但是可以实现输入（传入）int类型
    searchbst(t, 5);
    printf("\n");
